...
IMPLEMENT_SUBCLASS(GlutDisplay,"TestGlutDisplay")
      ...
      if (myShaders != 0) {
         ...
         while (item != 0 && ok) {
            ...
            if (pair != 0) {
               ...
               if (shdr != 0) ok = shdr->createAndLinkShaders();
            }
            item = item->getNext();
         }
         if (!ok) {
...
//    Graphic for the select ID is not found.
//-----------------------------------------------------------------------------
BasicGL2_0::Graphic* GlutDisplay::findSelected(const GLint hits, const GLuint sbuff[], const int item)
{
   Graphic* sel = 0;
   GLuint id = 0;
